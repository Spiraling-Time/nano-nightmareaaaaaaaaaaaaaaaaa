	...
	if overall_mode == "IDLE":
		...
		if leg_mode == "WALK" and prev_scale_x == scale.x:
			...
			if leg_height_resetter.is_stopped():
				...
				#print("restarting")
			if upper_leg1.number_of_bots >= 1 and upper_leg2.number_of_bots >= 1: velocity.x += (speed * delta * 6000000 * clamp(0.001, upper_leg1.number_of_bots / upper_leg1.max_number_of_bots, 1)*clamp(0.001, upper_leg2.number_of_bots / upper_leg2.max_number_of_bots, 1))
			else: velocity.x = 0
